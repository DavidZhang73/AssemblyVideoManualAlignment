...
LOG_PATH = "/data/jiahao/logs"
WANDB_PROJECT = "AssemblyVideoManualAlignment"
MODEL_CHECKPOINT_PATH = os.path.join(LOG_PATH, WANDB_PROJECT)
LOG_PATH = os.path.join(LOG_PATH, "wandb")

if __name__ == "__main__":
    print("WandDB Project Name:", WANDB_PROJECT)
    print("WandDB Log Path:", LOG_PATH)
    # get run logs
    ...
    for run_id, log_path in run_id_to_log_path_map.items():
        if run_id not in online_run_id_to_name_map:
            ...
            keep_count += 1
    print(f"Runs:\t\t{keep_count} kept {delete_count} deleted")
    ...
    for run_id, model_checkpoint_path in run_id_to_model_checkpoint_path_map.items():
        if run_id not in online_run_id_to_name_map:
            ...
            # print(f"Keeping {online_run_id_to_name_map[run_id]}({run_id})")
            keep_count += 1
    print(f"Checkpoints:\t{keep_count} kept {delete_count} deleted")
