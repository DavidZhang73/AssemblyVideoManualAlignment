...
class OptimalTransportFcn(torch.autograd.Function):
    ...
    def backward(ctx, dJdP):
        ...
        if ctx.method == "block":
            ...
            v1 = torch.cholesky_solve(vHAt1.view(B, H - 1, 1), block_11).view(B, H - 1) - torch.einsum(
                "bi,bji->bj", vHAt2, block_12
            )
...
if __name__ == "__main__":
    ...
    test = gradcheck(
        f, (torch.exp(-1.0 * M), None, None, 1.0, 1.0e-6, 1000, True, "block"), eps=1e-6, atol=1e-3, rtol=1e-6
    )
    print(test)
